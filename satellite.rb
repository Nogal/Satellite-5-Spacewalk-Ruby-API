...
module Satellite
	...
	class Channel
		class Software
			def self.listAllPackages(channel)
			#Description
			#	Lists all packages in the channel, regardless 
			#	of package version, between the given dates.
			#Parameters
			#	string sessionKey
			...
			#			string last_modified - (Deprecated)
				...
				begin
					info = $a_client.call('channel.software.listAllPackages', $a_key, channel)
				rescue XMLRPC::FaultException => e
					error_string = e.faultString.scan(/redstone.xmlrpc.XmlRpcFault: (.+$)/)
					puts error_string
					exit 2
	...
	class Packages
		def self.getPackage(package_id)
		#Description
		...
		#	binary object - package file
				begin
					info = $a_client.call('packages.getPackage', $a_key, package_id)
				rescue XMLRPC::FaultException => e
					error_string = e.faultString.scan(/redstone.xmlrpc.XmlRpcFault: (.+$)/)
					puts error_string
					exit 2
